/* TanStack Start currently loses server-function handler generics across its middleware boundary. */
...
export const fetchUnifiedUsers = protectedServerFn("GET")
  .inputValidator(listSchema)
  .handler((async (ctx: unknown): Promise<UnifiedUsersPage> => {
...
export const fetchUnifiedUserDetails = protectedServerFn("GET")
  .inputValidator(userIdSchema)
  .handler((async (ctx: unknown): Promise<UnifiedUserDetails> => {
    const { data } = ctx as { data: z.infer<typeof userIdSchema> };
    const supabase = getSupabaseServerClient({ admin: true });
    ...
    const activityPromise = listByUser("user_activity_events", data.userId, 50);
    ...
    const auditTargets = [data.userId, ...accountIds];
...
export const executeCentralAdminCommand = protectedServerFn("POST")
  .inputValidator(commandSchema)
  .handler((async (
    ctx: unknown,
  ): Promise<{ ok: boolean; commandId?: string; message?: string }> => {
    const { data } = ctx as { data: z.infer<typeof commandSchema> };
...
export const createCentralAccessLink = protectedServerFn("POST")
  .inputValidator(accessLinkSchema)
  .handler((async (
    ctx: unknown,
  ): Promise<{ url: string; expiresAt: string | null }> => {
    const { data } = ctx as { data: z.infer<typeof accessLinkSchema> };
    const account = await resolveProductAccount(data.userId, data.productSlug);
    const { data: result, error } = await getSupabaseServerClient({
      admin: true,
      ...
      body: {
        product: data.productSlug,
        kind: data.productSlug === "wagoo" ? "promo" : "complimentary",
        external_user_id: account.external_user_id,
        organization_id: account.organization_id,
...
export const fetchNotifications = protectedServerFn("GET")
  .inputValidator(z.object({ includeArchived: z.boolean().default(false) }))
  .handler((async (ctx: unknown): Promise<Notification[]> => {
    const { data } = ctx as { data: { includeArchived: boolean } };
...
export const mutateNotification = protectedServerFn("POST")
  .inputValidator(notificationMutationSchema)
  .handler((async (ctx: unknown): Promise<{ ok: true }> => {
    const { data } = ctx as {
      data: z.infer<typeof notificationMutationSchema>;
    };
    const patch =
      data.action === "read"
...
export const fetchSupabasePublicConfig = protectedServerFn("GET")
  .inputValidator(z.object({}))
  .handler((async (): Promise<{ url: string; anonKey: string }> => {
